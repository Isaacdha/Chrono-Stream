...
with tab3:
    max_lags = min(60, max(1, len(values) // 2 - 1))
    ...
    def correlation_frame(
        coefficients: np.ndarray, confidence: np.ndarray
    ) -> pd.DataFrame:
        return pd.DataFrame(
            {
                "Lag": np.arange(len(coefficients)),
                "Correlation": coefficients,
                "Lower reference": confidence[:, 0] - coefficients,
                "Upper reference": confidence[:, 1] - coefficients,
            }
        )

    def correlation_chart(frame: pd.DataFrame, title: str) -> alt.LayerChart:
        band = (
            alt.Chart(frame)
            .mark_area(opacity=0.16, color="#8ecae6")
            .encode(
                ...
                y2="Upper reference:Q",
            )
        )
        stems = (
            alt.Chart(frame)
            .mark_rule(color="#fb8500")
            .encode(
                x="Lag:Q",
                y="Correlation:Q",
                y2=alt.datum(0),
        ...
        points = (
            alt.Chart(frame)
            .mark_point(filled=True, color="#fb8500", size=35)
            .encode(x="Lag:Q", y="Correlation:Q")
        )
        return alt.layer(band, stems, points).properties(title=title, height=300)

    correlation_columns = st.columns(2)
    correlation_columns[0].altair_chart(
        correlation_chart(
            correlation_frame(acf_values, acf_confidence),
            "Autocorrelation (ACF)",
        ),
        width="stretch",
    )
    correlation_columns[1].altair_chart(
        correlation_chart(
            correlation_frame(pacf_values, pacf_confidence),
            "Partial autocorrelation (PACF)",
        ),
        width="stretch",
    )
    st.caption(
        "Shading shows approximate pointwise 95% references (Bartlett for ACF; "
        "normal approximation for PACF). These are exploratory guides, not simultaneous tests."
    )
    if len(values) >= 12:
        ...
        if p_value < 0.05:
            st.success(
                "The ADF test rejects its unit-root null at the 5% level. This supports "
                "a non-unit-root interpretation under the test specification, but does not "
                "by itself establish stationarity when trends, breaks, or other violations are present."
            )
        else:
            st.info(
