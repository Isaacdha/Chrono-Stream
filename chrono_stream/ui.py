...
from .evaluation import ACCURACY_METRIC_KEYS, evaluate_and_forecast
...
from .methods.statistical.box_jenkins_pipeline import NoEligibleModelError
...
def _render_method_popover(model_id: str) -> None:
    ...
    information = METHOD_INFORMATION[spec.method_research_key]
    title = spec.display_name
    statistical_keys = spec.statistical_test_keys
...
def _render_scholarly_popover(model_id: str) -> None:
    ...
    spec = get_method(model_id)
    research_key = spec.method_research_key
    information = METHOD_INFORMATION[research_key]
    title = spec.display_name
    statistical_keys = spec.statistical_test_keys
    complete_note = copy_ready_method_note(research_key, title)
    ...
    with st.popover(
        ...
        with research_tabs[0]:
            ...
            st.code(
                METHOD_LITERATURE_REVIEWS[research_key], language=None, wrap_lines=True
            )
...
METRIC_GROUPS = (
    ("Original scale", ("MAE", "RMSE")),
    ("Percentage", ("MAPE", "sMAPE", "WAPE")),
    ("Benchmark scaled", ("MASE", "RMSSE")),
)


def _render_metric_practical_popover(key_prefix: str) -> None:
    """Render formulas, interpretation, software behavior, and limitations."""
    with st.popover(
        "! Metrics",
        help="Accuracy metric explanations",
        width="content",
        key=f"{key_prefix}_metric_information",
    ):
        st.markdown("### Forecast accuracy metrics")
        st.caption(
            "Lower is better for every displayed metric. No single measure represents "
            "every forecasting objective or data condition."
        )
        group_tabs = st.tabs([label for label, _keys in METRIC_GROUPS])
        for group_tab, (_label, metric_keys) in zip(
            group_tabs, METRIC_GROUPS, strict=True
        ):
            with group_tab:
                for metric_key in metric_keys:
                    information = METRIC_INFORMATION[metric_key]
                    with st.container(border=True):
                        st.markdown(f"#### {information.display_name}")
                        st.code(information.formula, language=None, wrap_lines=True)
                        st.markdown("**How it works**")
                        st.write(information.how_it_works)
                        st.markdown("**In Chrono Stream**")
                        st.write(information.chrono_stream)
...
def _render_metric_scholarly_popover(key_prefix: str) -> None:
    """Render complete metric literature, references, and text downloads."""
    with st.popover(
        "? Research",
        help="Metric literature reviews and references",
        width="content",
        key=f"{key_prefix}_metric_research",
    ):
        st.markdown("### Forecast accuracy metric literature")
        review_tab, references_tab, files_tab = st.tabs(
            ["Metric reviews", "References", "Files"]
        )
        with review_tab:
            review_tabs = st.tabs(list(ACCURACY_METRIC_KEYS))
            for metric_tab, metric_key in zip(
                review_tabs, ACCURACY_METRIC_KEYS, strict=True
            ):
                with metric_tab:
                    information = METRIC_INFORMATION[metric_key]
                    st.markdown(f"#### {information.display_name}")
        ...
        with files_tab:
            selected_metric = st.selectbox(
                "Individual metric note",
                ACCURACY_METRIC_KEYS,
                format_func=lambda key: METRIC_INFORMATION[key].display_name,
                key=f"{key_prefix}_metric_note_choice",
            )
            st.download_button(
                "Download selected metric note (TXT)",
                data=copy_ready_metric_note(selected_metric),
                file_name=f"chrono_stream_{selected_metric.lower()}_metric_note.txt",
                mime="text/plain",
                icon=":material/download:",
                on_click="ignore",
                width="stretch",
                key=f"{key_prefix}_metric_note_download",
            )
            st.download_button(
                "Download complete metric handbook (TXT)",
                data=copy_ready_metric_handbook(),
                file_name="chrono_stream_accuracy_metric_handbook.txt",
                mime="text/plain",
                icon=":material/download:",
                on_click="ignore",
                width="stretch",
                key=f"{key_prefix}_metric_handbook_download",
            )


def render_metric_reference_actions(key_prefix: str = "accuracy") -> None:
    """Render the shared practical and scholarly accuracy-metric controls."""
    st.markdown(
        """
        <style>
    ...
    with st.container(
        ...
        _render_metric_practical_popover(key_prefix)
        _render_metric_scholarly_popover(key_prefix)
...
def _forecast_chart(result: dict[str, Any]) -> alt.LayerChart:
    ...
    interval = (
        ...
        .encode(
            ...
            tooltip=[
                alt.Tooltip("Date:T"),
                alt.Tooltip("Lower interval:Q", format=",.4f"),
                alt.Tooltip("Upper interval:Q", format=",.4f"),
            ],
        )
    )
...
def _result_csv(result: dict[str, Any]) -> bytes:
    ...
    history["Lower interval"] = np.nan
    history["Upper interval"] = np.nan
...
def _correlation_chart(records: list[dict[str, Any]], title: str) -> alt.LayerChart:
    ...
    if "outside_reference_band" not in frame and "significant" in frame:
        frame["outside_reference_band"] = frame["significant"]
    band = (
        alt.Chart(frame)
    ...
    stems = (
        ...
        .encode(
            ...
            tooltip=[
                ...
                alt.Tooltip("value:Q", format=".4f"),
                alt.Tooltip(
                    "outside_reference_band:N", title="Outside heuristic band"
                ),
            ],
        )
    )
...
def _display_box_jenkins_details(
    ...
    candidate_warning = final_details.get("candidate_warning")
    ...
    with pipeline_tab:
        ...
        if order_correlations.get("acf") and order_correlations.get("pacf"):
            ...
            st.caption(
                order_correlations.get(
                    "reference_band_method",
                    "The displayed pointwise bands are heuristic references, not "
                    "universal lag-significance tests.",
                )
            )
...
def display_model_result(result: dict[str, Any]) -> None:
    result_header = st.columns([12, 4], gap="small", vertical_alignment="center")
    with result_header[0]:
        st.subheader("Results")
    with result_header[1]:
        render_metric_reference_actions(f"{result['model_id']}_result")

    metrics = result["metrics"]
    scale_columns = st.columns(4)
    scale_columns[0].metric("Holdout MAE", _format_metric(metrics.get("MAE")))
    scale_columns[1].metric("Holdout RMSE", _format_metric(metrics.get("RMSE")))
    scale_columns[2].metric("Holdout MASE", _format_metric(metrics.get("MASE")))
    scale_columns[3].metric("Holdout RMSSE", _format_metric(metrics.get("RMSSE")))
    percentage_columns = st.columns(3)
    percentage_columns[0].metric(
        "Holdout MAPE", _format_metric(metrics.get("MAPE"), percent=True)
    )
    percentage_columns[1].metric(
        "Holdout sMAPE", _format_metric(metrics.get("sMAPE"), percent=True)
    )
    percentage_columns[2].metric(
        "Holdout WAPE", _format_metric(metrics.get("WAPE"), percent=True)
    )
    metric_context = result.get("metric_context", {})
    scale_benchmark = metric_context.get("scale_benchmark")
    st.caption(
        f"Metrics use the final {result['holdout']} observed periods as an out-of-sample "
        "holdout."
        + (
            f" MASE and RMSSE use {scale_benchmark.lower()} from pre-holdout training data."
            if scale_benchmark
            else " Refit this legacy saved result to calculate MASE, RMSSE, and WAPE."
        )
    )
    _display_tuning_details(result)
    interval_details = result.get("model_details", {})
    interval_method = interval_details.get("interval_method", "Not reported")
    if interval_details.get("interval_available", True):
        st.caption(f"Forecast interval: {interval_method}.")
        interval_warning = interval_details.get("interval_warning")
        if interval_warning:
            st.caption(interval_warning)
    else:
        st.warning(
            f"Forecast interval unavailable. {interval_details.get('interval_warning', '')}".strip()
        )
...
def render_model_page(model_id: str) -> None:
    ...
    if len(data) <= spec.minimum_observations:
        st.error(
            f"{spec.display_name} needs at least "
            f"{spec.minimum_observations + 1} observations when a holdout is used."
        )
        st.stop()
    ...
    maximum_holdout = len(data) - spec.minimum_observations
    if not 1 <= holdout <= maximum_holdout:
        st.error(
            f"The shared holdout is {holdout}, but {spec.display_name} permits at most "
            f"{maximum_holdout} with this data. Reduce the holdout on Data Input & "
            "Settings; it will not be silently changed for one model."
        )
        st.stop()
    seasonal_period = int(st.session_state.get("seasonal_period", 12))
    metric_scale_period = int(
        st.session_state.get("metric_scale_period", seasonal_period)
    )
    ...
    if model_id in {"arima", "sarima"}:
        ...
        parameters = spec.render_parameters(len(data) - holdout, seasonal_period)
        submitted = st.button(
    ...
    if submitted:
        with st.spinner(
            f"Training {spec.display_name} and running the holdout evaluation..."
        ):
            try:
                result = evaluate_and_forecast(
                    ...
                    params=parameters,
                    frequency=frequency,
                    metric_scale_period=metric_scale_period,
                )
