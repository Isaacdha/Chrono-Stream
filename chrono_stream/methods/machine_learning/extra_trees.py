"""Extremely randomized tree-ensemble forecasting from causal lag features."""
...
def forecast(
    ...
    **_: Any,
    ...
    """Fit an Extra Trees regressor and recursively build future rows."""
    ...
    if n_estimators < 10:
        raise ValueError("Extra Trees needs at least 10 trees.")
    ...
    if automatic:
        ...
        if not np.isfinite(search.best_score_):
            raise ValueError("No Extra Trees candidate produced a finite CV score.")
    ...
    details = {
        **feature_details(config, names, frequency),
        ...
        "bootstrap": False,
        "split_randomization": "Random thresholds as well as random feature subsets",
        "feature_importances": {
            name: float(value)
        ...
        "extrapolation_warning": (
            "Extra Trees averages learned leaves and generally extrapolates trends poorly."
        ),
    }
...
def render_parameters(data_length: int, seasonal_period: int) -> dict[str, Any]:
    """Render Extra Trees controls."""
    import streamlit as st

    automatic = st.toggle(
        "Automatically tune ensemble complexity with expanding-window CV", value=True
    )
    ...
    parameters.update(
        render_feature_controls(data_length, seasonal_period, key_prefix="extra_trees")
    )
    return parameters
...
SPEC = MethodSpec(
    ...
    display_name="Extra Trees Regression",
    icon="🌴",
    navigation_group="Machine Learning",
    description="An ensemble that randomizes split thresholds and feature subsets on causal lag rows.",
    guidance="Extra randomization can reduce variance efficiently, but its leaf averages still cannot project an unseen trend.",
    forecast=forecast,
    render_parameters=render_parameters,
    multi_step_strategy="Recursive",
    interval_capability="Descriptive in-sample residual band; not a native ensemble interval and unavailable when degenerate",
