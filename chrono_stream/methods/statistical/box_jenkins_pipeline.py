...
DEFAULT_MAX_REGULAR_DIFFERENCING = 2
DEFAULT_MAX_SEASONAL_DIFFERENCING = 2
DEFAULT_MAX_AR_MA_ORDER = 6
DEFAULT_MAX_SEASONAL_AR_MA_ORDER = 3
DEFAULT_MAX_TOTAL_ORDER = 12
CANDIDATE_WARNING_THRESHOLD = 150
...
def _seasonal_difference_order(
    ...
    if normalised_method in {
        "acf",
        "acf significance",
        "seasonal correlation",
        "seasonal lag acf heuristic",
    }:
        selected = _acf_seasonal_difference_order(values, period, maximum)
        return selected, [
            {
                "method": "Seasonal-lag ACF heuristic",
                "selected_D": selected,
                "maximum_D": maximum,
            }
        ]
...
def _correlation_records(values: np.ndarray, maximum_lag: int) -> dict[str, Any]:
    ...
    return {
        ...
        "reference_level": 0.95,
        "reference_band_method": (
            "Pointwise white-noise heuristic ±1.96/sqrt(n); not a universal "
            "lag-significance test"
        ),
...
def _guided_components(
    ...
    flagged_p = [
        ...
        if row["outside_reference_band"] and int(row["lag"]) <= maximum_p
    ]
    flagged_q = [
        int(row["lag"])
        for row in correlations["acf"]
        if row["outside_reference_band"] and int(row["lag"]) <= maximum_q
    ]
    flagged_P = [
        int(row["lag"]) // period
        for row in correlations["pacf"]
        if row["outside_reference_band"]
    ...
    return (
        _bounded_neighbours(flagged_p, maximum_p),
        _bounded_neighbours(flagged_q, maximum_q),
        _bounded_neighbours(flagged_P, maximum_P),
        _bounded_neighbours(flagged_Q, maximum_Q),
    )
...
def _order_values(value: Any, label: str, maximum: int | None) -> list[int]:
    if isinstance(value, str):
        parts = value.replace(";", ",").split(",")
        try:
    ...
    if any(item < 0 for item in selected):
        raise ValueError(f"Every {label} value must be zero or greater.")
    if maximum is not None and any(item > maximum for item in selected):
        raise ValueError(
            f"Every {label} value must be between 0 and {maximum}. Enable "
            "unbounded orders to use larger values."
        )
    return selected


def _validate_order_limits(params: dict[str, Any], seasonal: bool) -> None:
    """Keep API inputs consistent with the bounded/unbounded UI contract."""
    unbounded = bool(params.get("allow_unbounded_orders", False))
    scalar_limits: list[tuple[str, str, int]] = [
        ("d", "d", DEFAULT_MAX_REGULAR_DIFFERENCING),
        ("max_d", "maximum d", DEFAULT_MAX_REGULAR_DIFFERENCING),
        ("p", "p", DEFAULT_MAX_AR_MA_ORDER),
        ("max_p", "maximum p", DEFAULT_MAX_AR_MA_ORDER),
        ("q", "q", DEFAULT_MAX_AR_MA_ORDER),
        ("max_q", "maximum q", DEFAULT_MAX_AR_MA_ORDER),
        ("max_order", "maximum total order", DEFAULT_MAX_TOTAL_ORDER),
    ]
    if seasonal:
        scalar_limits.extend(
            [
                ("D", "D", DEFAULT_MAX_SEASONAL_DIFFERENCING),
                ("max_D", "maximum D", DEFAULT_MAX_SEASONAL_DIFFERENCING),
                ("P", "P", DEFAULT_MAX_SEASONAL_AR_MA_ORDER),
                ("max_P", "maximum P", DEFAULT_MAX_SEASONAL_AR_MA_ORDER),
                ("Q", "Q", DEFAULT_MAX_SEASONAL_AR_MA_ORDER),
                ("max_Q", "maximum Q", DEFAULT_MAX_SEASONAL_AR_MA_ORDER),
            ]
        )
    for key, label, bounded_maximum in scalar_limits:
        if key not in params:
            continue
        try:
            selected = int(params[key])
        except (TypeError, ValueError) as exc:
            raise ValueError(f"{label} must be an integer.") from exc
        if selected < 0:
            raise ValueError(f"{label} must be zero or greater.")
        if not unbounded and selected > bounded_maximum:
            raise ValueError(
                f"{label} cannot exceed {bounded_maximum} unless unbounded orders are enabled."
            )

    list_maximum = None if unbounded else DEFAULT_MAX_AR_MA_ORDER
    seasonal_list_maximum = (
        None if unbounded else DEFAULT_MAX_SEASONAL_AR_MA_ORDER
    )
    if "manual_p_values" in params:
        _order_values(params["manual_p_values"], "p", list_maximum)
    if "manual_q_values" in params:
        _order_values(params["manual_q_values"], "q", list_maximum)
    if seasonal and "manual_P_values" in params:
        _order_values(params["manual_P_values"], "P", seasonal_list_maximum)
    if seasonal and "manual_Q_values" in params:
        _order_values(params["manual_Q_values"], "Q", seasonal_list_maximum)


def _bounded_grid_count(limits: tuple[int, ...], maximum_order: int) -> int:
    """Count bounded nonnegative order tuples without constructing the full grid."""
    dimensions = len(limits)
    total = 0
    for mask in range(1 << dimensions):
        shifted_order = maximum_order
        selected_limits = 0
        for index, limit in enumerate(limits):
            if mask & (1 << index):
                shifted_order -= limit + 1
                selected_limits += 1
        if shifted_order < 0:
            continue
        combinations = math.comb(shifted_order + dimensions, dimensions)
        total += -combinations if selected_limits % 2 else combinations
    return total


def _selected_grid_count(
    value_sets: tuple[list[int], ...], maximum_order: int
) -> int:
    """Count candidate-list combinations subject to the total-order limit."""
    subtotal_counts = {0: 1}
    for values in value_sets:
        next_counts: dict[int, int] = {}
        for subtotal, ways in subtotal_counts.items():
            for value in values:
                combined = subtotal + value
                if combined <= maximum_order:
                    next_counts[combined] = next_counts.get(combined, 0) + ways
        subtotal_counts = next_counts
    return sum(subtotal_counts.values())


def candidate_search_space_size(params: dict[str, Any], *, seasonal: bool) -> int:
    """Return the maximum number of candidate models implied by an order setup."""
    _validate_order_limits(params, seasonal)
    ...
    if strategy in {"manual", "manual order"}:
        return 1

    maximum_order = max(0, int(params.get("max_order", 6)))
    unbounded = bool(params.get("allow_unbounded_orders", False))
    if strategy in {"manual candidate list", "manual list"}:
        order_maximum = None if unbounded else DEFAULT_MAX_AR_MA_ORDER
        seasonal_order_maximum = (
            None if unbounded else DEFAULT_MAX_SEASONAL_AR_MA_ORDER
        )
        p_values = _order_values(
            params.get("manual_p_values", "0,1"), "p", order_maximum
        )
        q_values = _order_values(
            params.get("manual_q_values", "0,1"), "q", order_maximum
        )
        P_values = (
            _order_values(
                params.get("manual_P_values", "0,1"),
                "P",
                seasonal_order_maximum,
            )
            if seasonal
        ...
        Q_values = (
            _order_values(
                params.get("manual_Q_values", "0,1"),
                "Q",
                seasonal_order_maximum,
            )
            if seasonal
        ...
        return _selected_grid_count(
            (p_values, q_values, P_values, Q_values), maximum_order
        )

    limits = (
        max(0, int(params.get("max_p", 3))),
        max(0, int(params.get("max_q", 3))),
        max(0, int(params.get("max_P", 1))) if seasonal else 0,
        max(0, int(params.get("max_Q", 1))) if seasonal else 0,
    )
    return _bounded_grid_count(limits, maximum_order)


def candidate_search_warning(candidate_count: int) -> str | None:
    """Describe the soft performance warning for a large candidate search."""
    if candidate_count <= CANDIDATE_WARNING_THRESHOLD:
        return None
    return (
        f"The configured search can evaluate up to {candidate_count:,} tentative candidate "
        f"models, above the {CANDIDATE_WARNING_THRESHOLD}-model warning threshold. "
        "Model selection may be very slow and use substantial memory."
    )
...
def _heteroskedasticity_diagnostic(
    ...
    auxiliary_observations = len(residuals) - lags
    if auxiliary_observations <= model_df:
        return {
            "method": "ARCH LM",
            "lags": lags,
            "model_degrees_of_freedom": model_df,
            ...
            "passed": False,
            "error": (
                "ARCH LM has insufficient auxiliary-regression observations after "
                "the configured model-parameter correction."
            ),
        }
    try:
    ...
    return {
        ...
        "model_degrees_of_freedom": model_df,
        "auxiliary_observations": auxiliary_observations,
        "lm_scaling": "(auxiliary_observations - model_degrees_of_freedom) * R-squared",
        "chi_square_degrees_of_freedom": lags,
...
def _base_failure_details(
    ...
    return {
        ...
        "candidate_models_configured": candidate_count,
    ...
    *,
    ...
    if strategy not in {
        ...
        if not seasonal:
            full_candidates = [
                (order, (0, 0, 0, 0))
                for order, _seasonal_order in full_candidates
            ]
    ...
    fallback_enabled = bool(params.get("guided_fallback", True))
    candidate_count = len(primary_candidates) + (
        len(fallback_candidates) if fallback_enabled else 0
    )
    candidate_warning = candidate_search_warning(candidate_count)
    if candidate_warning:
        warnings.warn(candidate_warning, RuntimeWarning, stacklevel=2)
