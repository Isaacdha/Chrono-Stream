...
if raw_data is not None and not raw_data.empty:
    with st.container(border=True):
        ...
        if prepared is not None:
            ...
            metric_scale_period = settings[3].number_input(
                "MASE/RMSSE scale period",
                min_value=1,
                max_value=max(1, len(prepared) - int(holdout) - 1),
                value=min(
                    default_season,
                    max(1, len(prepared) - int(holdout) - 1),
                ),
                step=1,
                help=(
                    "Use 1 for an ordinary naive benchmark or a seasonal period such "
                    "as 12 for a seasonal-naive benchmark. Scaling uses training data only."
                ),
            )
            st.caption(
                "MASE and RMSSE use the selected scale period on pre-holdout training "
                "observations. This setting affects evaluation only, not model fitting."
            )
            ...
            if st.button("Save data and settings", type="primary", width="stretch"):
                ...
                st.session_state["metric_scale_period"] = int(metric_scale_period)
