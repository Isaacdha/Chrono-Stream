...
if 'filtered_df' in st.session_state:
    st.warning("Dataframe already exists in session state, please continue to next page or reupload to change the data.")
                ...
                try:
                    ...
                    if df[date_column].dtype == 'datetime64[ns]':
                        if date_column and value_column and date_column != value_column:
                            ...
                            col1, col2 = st.columns([5,5], gap = 'small')
                            with col1:
                                st.dataframe(df[[date_column, value_column]], use_container_width=True,
                                             height=300)
                            ...
                            with col2:
                                ...
                                if st.button("💾 Confirm & Save Data"):
                                    ...
                                    st.session_state['filtered_df'] = new_df
                                    st.success("Dataframe has been filtered and saved to session state.")
                                    if 'filtered_df' in st.session_state and 'unlocked' not in st.session_state:
                                        button = st.button("🔐 Unlock Features")
                                        if button:
                                            st.success("Features unlocked.")
...
if 'forecast_template' in st.session_state:
    forecast = True
else:
    forecast = False
...
with st.container(border = True):
    if 'filtered_df' in st.session_state:
        st.subheader("⚙️ Forecast Options")
        forecast = st.toggle("Use Method to Forecast Data?", value=forecast)
        if forecast == True:
            col1, col2 = st.columns(2)
            with col1:
        ...
        if 'forecast_template' in st.session_state:
            st.success(f"Forecasting enabled and set for {st.session_state['forecast_template'].shape[0]} periods.")
